...
class MemberPositionCalculator {
  static areStatesSame(state1, state2) {
    return (
      ...
      state1.directionOverride == state2.directionOverride &&
  ...
  static getState(member, count) {
    ...
    let delta =
      newState.deltaX != undefined && newState.deltaY != undefined
    ...
    return {
      ...
      direction: Utils.coalesce(newState.directionOverride, newState.direction),
      directionOverride: newState.directionOverride,
      deltaX: delta.deltaX,
      deltaY: delta.deltaY,
      count: count,
    };
  ...
  static doAction(currentState, action) {
    ...
    return {
      ...
      direction: Utils.coalesce(action.directionOverride, action.direction),
  ...
  static undoAction(currentState, previousState, action) {
    ...
    return {
      ...
      directionOverride: previousState.directionOverride,
  ...
  static stepBackward(member, currentState, steps) {
    ...
    for (let i = 0; i < steps; i++) {
      ...
      // currentState.
      if (
        !this.isBeginningOfDrill(member, newState) &&
  ...
  static needs8to5Adjustment(state) {
    if (
      state.strideType == StrideType.EightToFive &&
      state.deltaY > 0 &&
  ...
  static adjustedDeltaY(state) {
    if (state.deltaY > 0 && state.deltaX == 0) {
  ...
  static isBetweenHashes(state) {
    return (
      state.y >= FieldDimensions.farHashY &&
  ...
  static isEndOfDrill(member, currentState) {
    ...
    return (
      this.isAtFieldEdge(member, currentState) ||
